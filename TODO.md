...
- [x] 采用范围化业务授权：用户授权人可读结果、边界和条件，Agent 内部逐条 prepare、校验 fingerprint、commit 和验证，不再要求逐项重复确认（[ADR-0004](docs/decisions/0004-scoped-business-automation.md)）
...
- [x] 所有写操作仍要求显式 `--confirm` 并在调用前复核状态；`--confirm` 可断言精确单项指令或仍有效的范围策略，不要求用户确认内部 fingerprint
...
- [x] 通过 Playwright 申请勤務時間修正，并为变更增加预览、Agent 指纹校验和写后复核（支持单段勤務及可选单段休憩，或精确的「勤務時間を削除」申请）
- [x] 查询本人申请列表和详情，创建休假申请，并撤回可操作的本人申请
- [x] 为已批准的本人申请创建取消申请；绑定原申请、理由、审批路径和新申请编号；仅在授权的最终结果明确包含后续批准时复用管理员审批流程
...
- [x] 增加月次締め专用管理员列表与单件审阅，绑定成员月次汇总、逐日勤怠、警告和自动检查结果
- [x] 增加月次締め专用单件承认/差戻し prepare/commit；完整审阅内容纳入状态指纹
- [x] 为本人月次查询及管理员月次审阅增加跨月导航；按当前支付月/勤務月偏移定位，并在读取前验证目标勤務月
- [x] 等待申请筛选响应完成后解析，并支持分页查询及跨页详情定位
- [x] 单件承认或差戻し员工申请；写入前由 Agent 将对象、期间、内容、自动检查结果及详情指纹与精确指令或范围策略核对
- [x] 为勤務時間修正审批详情和预览增加结构化修改前/后对照，并将对照绑定进安全指纹
- [ ] 为现有范围化业务自动化增加持久化策略状态、可撤销策略、可导出的 dry-run 报告与审计日志

## 第四阶段：MCP 接入

- [x] 抽取 `FreeeService`，供 CLI 与 MCP 共享认证、后端和业务实现
- [x] 实现本地 STDIO MCP Server 和 27 个结构化工具
- [x] 为只读、预览和写入工具添加 MCP annotations
- [x] 打卡、月次提交/撤回和审批采用 prepare/commit 两阶段工具，并绑定 SHA-256 状态指纹
- [x] commit 工具要求 `confirm: true`、匹配 fingerprint 和服务端状态复核；Codex 对七个已审查业务 commit 使用单工具 `approve`，避免已授权运行被宿主逐项重复提示
...
- [ ] 按 [`docs/live-validation-checklist.md`](docs/live-validation-checklist.md) 逐项完成已实现功能的真实 freee / 真实 Agent 验收；每个写入都必须经过 prepare，并匹配精确指令或仍有效的范围策略
- [x] 在下一次真实本人申请中确认创建 commit 返回完整 JSON；已用 No. `10035` 验证
- [ ] 下次本来就需要差戻し本人申请时，确认 employee-history fallback 直接返回成功 JSON；不要为输出测试额外制造申请
- [x] 用 No. `10034` 验证已批准本人申请的取消 prepare/commit、新取消申请编号、管理员批准，以及原休假最终取消；已于 2026-08-14 完成，取消申请 No. `10039` 为 `承認済`，原申请 No. `10034` 为 `休暇取消済`
- [ ] 验收完成后仅记录版本、后端、对象编号/期间、指纹前 12 位和最终状态，不保存凭据、Cookie、员工表格或截图

## 安全规则

- 不向日志、终端错误、测试快照或 Git 文件输出任何 Token / Secret
- 默认只读；真实写操作必须由精确用户指令或明确范围策略授权，开发、测试、调查和模糊请求不构成授权
