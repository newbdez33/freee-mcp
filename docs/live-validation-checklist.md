...
- [x] **LV-W09 — Approved personal application cancellation.** On an approved application that genuinely needs cancellation, prepare the exact `取消申請`, review the original No./status/type/date/content, cancellation reason, route, and fingerprint, then commit once and verify exactly one new cancellation application. Approve that new No. only through a separate manager preview and explicit confirmation, then verify the original leave is cancelled. Validated on 2026-08-14 with the Playwright backend: original leave No. `10034`, cancellation application No. `10039`, cancellation fingerprint prefix `204af5d66f9d`, approval fingerprint prefix `da834481f88d`, final cancellation status `承認済`, and original application type `休暇取消済`.
...
- [x] **LV-R13 — Leave balances read.** Read the paid-holiday, special-holiday, and compensatory-holiday balances for the current employee, one visible member by ID, and one visible member by name; verify the parsed totals match the freee page, that the named lookup accepts a unique partial name, and that unknown or ambiguous names stop with the documented error. Validated on 2026-09-25 with Playwright `0.4.7`; no write was performed.
