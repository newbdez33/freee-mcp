...
9. For a `月次勤怠締め` application requiring manager action, call `freee_monthly_approvals_list`, then `freee_monthly_approval_review` for the exact No. The review automatically selects and verifies the application's work month on both the attendance monitor and employee attendance page; stop if navigation or verification is ambiguous. Present the applicant, period, monthly summary, every daily row, alerts, and automatic checks. Use `freee_monthly_approval_prepare_action`; call `freee_monthly_approval_commit_action` with `confirm: true` only when the current user message explicitly approves that exact No. and action after seeing the complete preview. For other employee applications, use the general approval list, detail, prepare, and commit tools. Before approving a `休暇`, every pending approval page must be checked for a pending `勤務時間修正` with the exact same applicant and target date. Process every matching `勤務時間修正` first, then reread and prepare the `休暇` approval again; never bypass `LEAVE_APPROVAL_BLOCKED_BY_WORK_TIME_CORRECTION`, and stop fail-closed on `LEAVE_APPROVAL_DEPENDENCY_UNCONFIRMED`. The commit repeats this check and reopens the exact target before any click. This dependency rule does not block returning a `休暇` or processing the `勤務時間修正` itself. Before summarizing or preparing a pending `勤務時間修正`, read its detail and present `workTimeChange.before` and `workTimeChange.after`; each null time means freee displayed `未入力`. If `workTimeChange` is null, state that a reliable comparison is unavailable and do not infer one from the flattened list content; it still blocks a same-applicant, same-date `休暇` approval while pending. All lists are paginated; start at page 1 and follow `pageCount` only as needed. If any fingerprint changes, prepare and present a fresh preview instead of writing.
...
- Never use an approval commit tool or command directly. First prepare, present the application identity, type, target date, content, reason, any structured before/after work-time comparison, automatic checks, requested action, and fingerprint, and wait for explicit approval in a new current message.
- Never use the general approval action tools for a monthly attendance closing application when the dedicated monthly approval tools are available. A monthly commit requires the dedicated review and must bind the applicant's monthly summary, daily attendance, alerts, automatic checks, requested action, and fingerprint.
