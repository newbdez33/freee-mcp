...
9. For a `月次勤怠締め` application requiring manager action, call `freee_monthly_approvals_list`, then `freee_monthly_approval_review` for the exact No. The selected attendance-monitor month must match the application's work month. Present the applicant, period, monthly summary, every daily row, alerts, and automatic checks. Use `freee_monthly_approval_prepare_action`; call `freee_monthly_approval_commit_action` with `confirm: true` only when the current user message explicitly approves that exact No. and action after seeing the complete preview. For other employee applications, use the general approval list, detail, prepare, and commit tools. All lists are paginated; start at page 1 and follow `pageCount` only as needed. If any fingerprint changes, prepare and present a fresh preview instead of writing.
