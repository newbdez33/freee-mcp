...
4. For API setup, run `auth configure` only after explicit approval. Prefer `system`, which stores Client Secret and Tokens in the System Keyring. In a Claude plugin installation, use the plugin-resolved CLI path and persistent data directory; never invent a repository-relative command.
...
For MCP, read `structuredContent` and report the meaningful result in the user's language. When a Claude plugin CLI command is necessary, use the exact command returned by MCP or this plugin-resolved form: `node "${CLAUDE_PLUGIN_ROOT}/scripts/plugin-cli.mjs" --plugin-data "${CLAUDE_PLUGIN_DATA}" <arguments>`. For source development only, run `npm run freee -- <arguments>` from the repository root. Read the JSON envelope from stdout or stderr.
